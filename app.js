/* REGNUM — the slowest of the six. Pure dissolve, no travel at all. */
(function(){
  var io=null,failsafe=null;
  var STEP=130, DUR=1000, EASE='cubic-bezier(.25,.6,.3,1)';
  function reveal(){
    ...
    if(!io){
      io=new IntersectionObserver(function(es){
        ...
        es.forEach(function(e){
          ...
          setTimeout(function(){
            el.style.opacity='1';
            var t=el.querySelector('.card-name');
            if(t) t.style.letterSpacing='.01em';
          },(n++)*STEP);
        });
      },{rootMargin:'0px 700px -5% 700px',threshold:.01});
    }
    [].forEach.call(document.querySelectorAll('.card:not([data-rv]),.gal-item:not([data-rv])'),function(el){
      ...
      var t=el.querySelector('.card-name');
      if(t){ t.style.letterSpacing='.16em'; t.style.transition='letter-spacing '+DUR+'ms '+EASE; }
      io.observe(el);
    });
    ...
    failsafe=setTimeout(function(){
      [].forEach.call(document.querySelectorAll('[data-rv]'),function(el){
        if(parseFloat(getComputedStyle(el).opacity)<.9){
          el.style.opacity='1';
          var t=el.querySelector('.card-name'); if(t) t.style.letterSpacing='.01em';
        }
      });
    },4000);
  }
